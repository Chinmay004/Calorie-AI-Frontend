  ...
  return (
    <div className="relative border rounded-lg overflow-hidden shadow-md p-4 bg-[#FAF4B7] group">
      <Link to={`/recipe/${recipe._id}`} className="block">
        ...
        <div className="p-2">
          <h3 className="text-lg text-center potta-one tracking-widest text-[#216e40]">
            {recipe.title}
          </h3>
        </div>
      </Link>
      ...
      <div className="absolute inset-0 bg-[#000000b9] bg-opacity-50 flex gap-4 items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        ...
        <Link
          ...
          className="px-4 py-2 rounded-lg font-semibold duration-300 flex items-center gap-2 bg-yellow-200 hover:bg-yellow-400 transition-transform active:scale-95"
        >
          <TbHandClick size={25} />
        </Link>
        <ShareButton />
...
RecipeCard.propTypes = {
  recipe: PropTypes.shape({
    _id: PropTypes.string.isRequired,
    title: PropTypes.string.isRequired,
    image: PropTypes.arrayOf(PropTypes.string), // Firebase URLs
  }).isRequired,
};
